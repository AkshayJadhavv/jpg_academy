...
export default function Hero() {
  ...
  return (
    <section id="home" className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
      {/* YouTube Video Background - Lighter overlay to see video clearly */}
      <div className="absolute inset-0 z-0 overflow-hidden">
        ...
        {/* Lighter overlay so video background is clearly visible */}
      ...
      >
        <motion.div variants={itemVariants} className="mb-4">
          <span className="inline-block font-sans text-xs tracking-[0.4em] uppercase text-[#D4AF37] border border-[#D4AF37]/30 px-5 py-2 rounded-full backdrop-blur-sm"
            style={{ background: 'rgba(212,175,55,0.08)' }}>
            Premier Design Institute · Baramati
          </span>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="mb-6">
          <img 
            ...
            className="inline-block h-16 w-auto sm:h-20 md:h-24 lg:h-28 object-contain mx-auto"
            style={{ filter: 'drop-shadow(0 4px 12px rgba(0,0,0,0.3))' }}
            onError={(e) => {
              // Fallback if image fails to load - show a gold circle with JBG text
              e.target.onerror = null;
              e.target.style.display = 'none';
              const parent = e.target.parentElement;
              const fallback = document.createElement('div');
              fallback.className = 'inline-flex items-center justify-center bg-[#D4AF37] text-black font-bold rounded-full h-16 w-16 sm:h-20 sm:w-20 md:h-24 md:w-24 lg:h-28 lg:w-28 text-2xl sm:text-3xl md:text-4xl mx-auto';
        ...
        >
          <a href="#contact" className="gold-btn flex items-center gap-2 w-full sm:w-auto justify-center px-6 py-3 bg-[#D4AF37] text-black font-bold rounded-full hover:bg-[#c5a22e] transition-all duration-300 shadow-lg">
            Enquire Now
          </a>
          <a
      ...
      >
        <span className="font-sans text-xs tracking-widest text-white/40 uppercase">Scroll</span>
        <motion.div
          animate={{ y: [0, 8, 0] }}
